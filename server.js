// server.js - Distrito Padel v8.1 con Postgres + Resend + Fix Timezone MST + Torneos - ESM
...
import { inicializarTablaTorneos } from './models/Torneo.js';
...
// Exportar pool para modulos
export { pool };

// RESEND
const resend = new Resend(process.env.RESEND_API_KEY);

// FUNCION PARA OBTENER FECHA LOCAL MST
...
async function initDB() {
  try {
    ...
    console.log('Tablas principales listas');
    
    // Inicializar tablas de torneos
    await inicializarTablaTorneos(pool);
  } catch (err) {
...
// ADMIN TOKEN
const ADMIN_TOKEN = process.env.ADMIN_TOKEN || 'distritoadmin23';
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!emailRegex.test(email)) {
      return res.status(400).json({ ok: false, msg: 'Email invalido' });
    }
    
    const telLimpio = telefono.replace(/\D/g, '');
    if (telLimpio.length !== 10) {
      return res.status(400).json({ ok: false, msg: 'Telefono debe tener 10 digitos' });
    }
...
app.post('/api/reservas/crear', async (req, res) => {
  ...
  try {
    ...
    if (userRes.rows.length === 0) {
      return res.json({ ok: false, msg: 'Token invalido' });
    }

    const user = userRes.rows[0];

    // FIX: Verificar limite de 7 dias usando fecha local MST
    ...
    if (diffDias > 7) {
      return res.json({ ok: false, msg: 'Solo puedes reservar hasta 7 dias adelante' });
    }
      ...
      await resend.emails.send({
        ...
        html: `
          ...
          <ul>
            ...
            <li><strong>Duracion:</strong> ${duracion}h</li>
...
app.post('/api/admin/promociones', verificarAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ ok: false, msg: 'Error al crear promocion' });
  }
});
...
app.delete('/api/admin/promociones/:id', verificarAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ ok: false, msg: 'Error al eliminar promocion' });
  }
});
...
app.get('/api/admin/reporte/dia', verificarAdmin, async (req, res) => {
  try {
    ...
    csv += `Hora,Duracion,Cancha,Cliente,Email,Telefono,Precio Base,Descuento %,Precio Final,Estado Pago,Metodo Pago\n`;
...
app.get('/api/admin/reporte/mes', verificarAdmin, async (req, res) => {
  try {
    ...
    csv += `Fecha,Hora,Duracion,Cancha,Cliente,Email,Telefono,Precio Base,Descuento %,Precio Final,Estado Pago,Metodo Pago\n`;
...
app.post('/api/admin/config', verificarAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ ok: false, msg: 'Error al actualizar configuracion' });
  }
});
...
app.listen(PORT, '0.0.0.0', async () => {
  db = await loadData();
  console.log(`Distrito Padel v8.1 ESM + Timezone Fix + Torneos corriendo en puerto ${PORT}`);
