...
// Configuración de precios en memoria (se puede sobreescribir desde /api/admin/config)
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Error email registro:', err.message);
    }
...
app.get('/api/reservas', async (req, res) => {
  try {
    ...
    console.error('Reservas GET error:', err.message);
...
app.post('/api/reservas/crear', async (req, res) => {
  ...
  try {
    ...
    // Fix límite 7 días usando fecha local MST
    ...
    if (horaDecimal < cambioTarifa && (horaDecimal + duracionNum) <= cambioTarifa) {
      ...
      precioBase =
        (horasAntes * config.precios.horaDia) +
        (horasDespues * config.precios.horaNoche);
    }
    ...
    await pool.query(
      `INSERT INTO reservas 
       (id, nombre, email, telefono, cancha, fecha, hora_inicio, duracion,
        precio, descuento, precio_base, estado, pagado)
    ...
    try {
      ...
      console.error('Error email reserva:', emailErr);
    }
    ...
    const [reservas, bloqueos] = await Promise.all([
      ...
      pool.query(
        `SELECT * FROM bloqueos
         WHERE fecha = $1 AND (cancha IS NULL OR cancha = $2)`,
        [fecha, parseInt(cancha)]
      )
    ]);
...
app.get('/api/promociones', async (req, res) => {
  try {
    ...
    console.error('Promociones GET error:', err.message);
    res.status(500).json({ ok: false, msg: 'Error al obtener promociones' });
  }
});
...
app.post('/api/admin/promociones', verificarAdmin, async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [id, fecha || null, descuento, horaInicio || null, horaFin || null]
    );
...
app.get('/api/bloqueos', async (req, res) => {
  try {
    ...
    console.error('Bloqueos GET error:', err.message);
    res.status(500).json({ ok: false, msg: 'Error al obtener bloqueos' });
  }
});
...
app.patch('/api/admin/reservas/:id/pagar', verificarAdmin, async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE reservas
       SET pagado = true, metodo_pago = $1, estado = 'pagada'
       WHERE id = $2`,
...
app.patch('/api/admin/reservas/:id/marcar-pendiente', verificarAdmin, async (req, res) => {
  try {
    await pool.query(
      `UPDATE reservas
       SET pagado = false, metodo_pago = NULL, estado = 'pendiente'
       WHERE id = $1`,
      [req.params.id]
    );
...
app.post('/api/admin/reservas/manual', verificarAdmin, async (req, res) => {
  try {
    ...
    if (finMin <= cambioMin) {
      ...
      precioBase =
        (antesMin / 60 * config.precios.horaDia) +
        (despuesMin / 60 * config.precios.horaNoche);
    }
    ...
    if (promo.rows.length > 0) {
      ...
      if (!p.hora_inicio || !p.hora_fin) {
        ...
        const promoInicio =
          parseInt(p.hora_inicio.split(':')[0]) * 60 +
          parseInt(p.hora_inicio.split(':')[1]);
        const promoFin =
          parseInt(p.hora_fin.split(':')[0]) * 60 +
          parseInt(p.hora_fin.split(':')[1]);
    ...
    await pool.query(
      `INSERT INTO reservas
       (id, nombre, email, telefono, cancha, fecha, hora_inicio, duracion,
        precio, descuento, precio_base, estado, pagado, es_manual)
...
app.get('/api/admin/reporte/dia', verificarAdmin, async (req, res) => {
  try {
    ...
    // Por método de pago
    const efectivo = rows
      .filter(r => r.metodo_pago === 'Efectivo' && r.pagado)
      .reduce((sum, r) => sum + parseFloat(r.precio || 0), 0);
    const tarjeta = rows
      .filter(r => r.metodo_pago === 'Tarjeta' && r.pagado)
      .reduce((sum, r) => sum + parseFloat(r.precio || 0), 0);
    const transferencia = rows
      .filter(r => r.metodo_pago === 'Transferencia' && r.pagado)
      .reduce((sum, r) => sum + parseFloat(r.precio || 0), 0);
...
app.get('/api/admin/reporte/mes', verificarAdmin, async (req, res) => {
  try {
    ...
    const reservas = await pool.query(
      `SELECT * FROM reservas
       WHERE fecha >= $1 AND fecha <= $2 AND estado != 'cancelada'
       ORDER BY fecha, hora_inicio`,
    ...
    const nombreMes = new Date(parseInt(año), parseInt(mes) - 1).toLocaleString('es-MX', {
      month: 'long'
    });
    ...
    csv += `Total Pagado,$${Math.round(totalPagado).toLocaleString()} MXN\n`;
    csv += `Total Pendiente,$${Math.round(totalPendiente).toLocaleString()} MXN\n`;
    csv += `Promedio por Reserva,$${Math.round(totalRecaudado / (totalReservas || 1)).toLocaleString()} MXN\n\n`;

    csv += `=== DESGLOSE POR MÉTODO DE PAGO ===\n`;
    csv += `Efectivo,$${Math.round(efectivo).toLocaleString()} MXN,${totalPagado ? Math.round(efectivo / totalPagado * 100) : 0}%\n`;
    csv += `Tarjeta,$${Math.round(tarjeta).toLocaleString()} MXN,${totalPagado ? Math.round(tarjeta / totalPagado * 100) : 0}%\n`;
    csv += `Transferencia,$${Math.round(transferencia).toLocaleString()} MXN,${totalPagado ? Math.round(transferencia / totalPagado * 100) : 0}%\n\n`;
